---
browser_title: For Developers - NCI Design System
...
1. Create your personal access token
   Follow the steps outlined in GitHub’s documentation for [creating a personal access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens).
   Ensure that, at a minimum, the required scopes are set to “repo,” “read:org,” and “workflow.”

2. Authenticate with GitHub
   Run the following command to authenticate with GitHub using the GitHub CLI:
   `gh auth login`

3. Log in to the @NCIOCPL Organizational Scope on npm
   Run the following command to log in to npm with the organizational scope and using the GitHub npm registry:
...
- Autoprefixing: NCIDS requires Autoprefixing your CSS with a specific .browserslistrc.
- Sass Load Paths: NCIDS requires Sass compilers use Load Paths that reference the /packages directory in the USWDS package
