...
NCI Design System (NCIDS) is built as an extension to the United States Web Design System (USWDS). In addition to its own components, NCIDS includes existing USWDS components for use within NCI web properties. USWDS-specific components can be found directly on the [USWDS Components page](https://designsystem.digital.gov/components/overview/) and are also listed below.
