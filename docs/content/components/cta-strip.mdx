...
usage: |
  ...
  ### When to use
  Use a **CTA strip** (in conjunction with either the [**Hero with CTA button**](./hero) or the [**Hero with tagline only**](./hero)) when a page needs to prominently display multiple CTAs. The **CTA strip** quickly directs users to important information on other pages.
  - Note: Use the Hero component variant “Hero with CTA strip” when adding the Hero and CTA strip components alongside each other. Learn more about [Hero component variations](./hero#variations). 
