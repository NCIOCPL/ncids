...
- `aria-controls` must be added to the collapsible section's button element
- `id` must be added to the collapsible section's list element and must match the value provided for `aria-controls`
- `aria-expanded` is updated automatically with javascript, it will be set to `true` if the collapsible section contains a `.hidden` class
