...
<NciDsJsInit path={props.pageContext.pagePath}>{`
	...
	accordions.forEach((element) => {
		// prevents re-initialization and ensures its only creating new accordions
		// that have not been modified yet by checking for usa-accordion__content
		if (element.children[1].classList.contains('usa-accordion__content')) return;
