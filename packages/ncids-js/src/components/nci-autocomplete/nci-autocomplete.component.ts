...
export class NCIAutocomplete {
	...
	protected acForm: HTMLFormElement | null;
	...
	/** Results announcer for screenreader */
	protected optionSelected: boolean;
	/** Autocomplete source */
	...
	private handleFormSubmission(): void {
		const formDetail = {
			inputtedText: this.newACInput.value,
			optionSelected: this.optionSelected,
		};
		this.acForm?.addEventListener('submit', function () {
			this.dispatchEvent(
	...
	private async handleInput() {
		// reset the fact that an option was selected
		this.optionSelected = false;
	...
	private selectOption(termHighlighted: boolean): void {
		if (termHighlighted && this.newACInput) {
			...
			const selectedOptionSpan = selectedOption?.querySelector('span');
			const stateSnapshot = {
				inputtedText: this.newACInput.value,
				optionValue: selectedOptionSpan?.getAttribute('aria-label') || '',
				optionSetSize: selectedOption?.getAttribute('aria-setsize') || '',
				optionIndex: selectedOption?.getAttribute('aria-posinset') || '',
			};
	...
	private handleOptionClick(e: Event): void {
		...
		if (this.newACInput && currSpan) {
			const stateSnapshot = {
				inputtedText: this.newACInput.value,
				optionValue: currSpan.getAttribute('aria-label') || '',
				optionSetSize: currOption.getAttribute('aria-setsize') || '',
				optionIndex: currOption.getAttribute('aria-posinset') || '',
			};
			this.newACInput.dispatchEvent(
				new CustomEvent('nci-autocomplete:optionSelected', {
					detail: stateSnapshot,
