...
// TODO had issues with `screen`, try again later as screen is better practice.
...
describe('NCI Footer', () => {
	...
	it('should not have sign up error messages on init', () => {
		const container = getExampleDOM();
		//document.body.append(container);
	...
	it('should have sign up error messages with invalid email submit', () => {
		const container = getExampleDOM();
		//document.body.append(container);

		const input = screen.getByLabelText('Enter your email address');
		//const input = getByLabelText(container, 'Enter your email address');
		const button = screen.getByRole('button', { name: /Sign up/i });
		//const button = getByRole(container, 'button', { name: /Sign up/i });
		...
		const query = screen.queryByRole('alert');
		//const query = queryByRole(container, 'alert');
		expect(query).toBeInTheDocument();
	});

	it('should not have sign up error messages with valid email submit', () => {
		const container = getExampleDOM();
		//document.body.append(container);
		...
		screen.debug(screen.queryByLabelText('Enter your email address'));

		const query = screen.queryByRole('alert');
		//const query = queryByRole(container, 'alert');
		// TODO expect(query).not.toBeInTheDocument();
	});
});
						...
						<div class='usa-sign-up'>
							...
							<form
								...
								<div class='usa-form-group usa-form-group--error'>
									...
									<div class='usa-error-message' id='email-error-message' role='alert'>
