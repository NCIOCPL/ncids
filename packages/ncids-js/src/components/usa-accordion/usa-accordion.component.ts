...
import { AccordionSection } from './usa-accordion-section';
/**
 * USA Accordion
...
export class USAAccordion {
	...
	/** array of AccordionSections */
	protected accordionSections: Array<AccordionSection>;
		...
		accordionHeadings.forEach((heading, index) => {
			...
			if (
				...
				const section: AccordionSection = {
					toggleStatus: () => this.toggleStatus(section),
					heading: heading as HTMLHeadingElement,
					contents: heading.nextElementSibling as HTMLDivElement,
					status: initOpen ? 'Open' : 'Closed',
				};
				this.accordionSections.push(section);
				return;
			}
	...
	public unregister() {
		...
		accordionHeadings.forEach((heading) => {
			...
			heading.innerHTML = heading.querySelector('button')!.textContent!;
	...
	/**
	 * Public utility to get the current array of AccordionSection
	 */
	public getSections(): Array<AccordionSection> {
		console.log(this.accordionSections);
		return this.accordionSections;
	}

	/**
	 * @param section AccordionSection to be toggled
	 * Toggles status of section
	 */
	public toggleStatus(section: AccordionSection) {
		const button = section.heading.firstChild as HTMLButtonElement;
		this.handleSectionToggle(
			button,
			section.status === 'Open' ? 'false' : 'true'
		);
	}

	/**
	 * @param button the section button being clicked to toggle it open or closed
	 * @param newState the state of the button we are changing it TO (if it is being opened, this value is true)
	 */
	public handleSectionToggle(button: HTMLButtonElement, newState: string) {
		button.setAttribute('aria-expanded', newState);
		// add hidden to targeted content
		const contentTargetId = button.getAttribute('aria-controls');
	...
	private handleAccordionToggleClick(e: Event): void {
		...
		this.handleSectionToggle(currBtn, newState);
	}
