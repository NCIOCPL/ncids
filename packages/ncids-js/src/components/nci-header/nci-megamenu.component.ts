...
export class NCIMegaMenu {
	...
	private static defaultOptions: NCIMegaMenuOptions = {
		menuClass: '.nci-header-nav__primary-item',
		menuButtonClass: '.nci-header-nav__primary-link',
	};
	...
	/**
	 * Megamenu creation based off the root HTMLelement on creation.
	 * Locates all parent LI elements to inject Megamenu wrapper and
	 ...
	 * the Megamenu as well as removes the link and adds the
	 * proper aria tags.
	 *
	...
	private activateButton(link: Element): void {
		...
		link.classList.remove('nci-header-nav__primary-link');
		link.classList.add('nci-header-nav__primary-button');
	}
	...
	private createMenuLayer(link: Element): Element {
		...
		menu.classList.add('nci-header-megamenu');
		...
		sectionHeader.classList.add('nci-header-megamenu__header');
	...
	private queryCollapseButtons(section: HTMLElement): NodeListOf<HTMLElement> {
		const selector = '.nci-header-nav-primary__button';
