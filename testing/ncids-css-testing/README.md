...
## Usage
1. Run `npm run start` to run Storybook locally
...
5. Run `npm run start` to start the Storybook server
6. Run `npm run backstop:test` to run tests locally with Storybook started
7. Run `npm run backstop:openReport` to open last test report.
...
6. Run `npm run backstop:test` to test the new scenarios
7. Run `npm run backstop:reference` with Storybook started to create reference files for new scenarios
...
3. Run `npm run start` to start the Storybook server
...
6. Run `npm run backstop:test` to ensure that the all tests pass.
7. Stop your Storybook server and commit your changes

## WSL
1. For WSL, preface all `npm run backstop:` commands with `CI=true`
    - e.g. `CI=true npm run backstop:reference`
...
## CI
1. GitHub Actions will automatically run `npm run test:css` for each push and pull request
