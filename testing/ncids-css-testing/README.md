...
## Usage
1. Run `pnpm run start` to run Storybook locally
...
5. Run `pnpm run start` to start the Storybook server
6. Run `pnpm run backstop:test` to run tests locally with Storybook started
7. Run `pnpm run backstop:openReport` to open last test report.
...
6. Run `pnpm run backstop:test` to test the new scenarios
7. Run `pnpm run backstop:reference` with Storybook started to create reference files for new scenarios
...
3. Run `pnpm run start` to start the Storybook server
4. To update existing files, you must run `npm run backstop:approve` with Storybook started
5. Review the list of files generated to ensure that it **ONLY** has modified those you expected.
6. Run `pnpm run backstop:test` to ensure that the all tests pass.
7. Stop your Storybook server and commit your changes

## WSL
1. For WSL, preface all `pnpm run backstop:` commands with `CI=true`
    - e.g. `CI=true pnpm run backstop:reference`
...
## CI
1. GitHub Actions will automatically run `pnpm run test:css` for each push and pull request
